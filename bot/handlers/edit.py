...
async def trip_addmedia_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("📷 Отправь фото или видео для добавления к сплаву:")
    await callback.answer()


@router.message(
    EditTrip.adding_media, F.content_type.in_([ContentType.PHOTO, ContentType.VIDEO])
)
...
async def trip_delmedia_callback(callback: types.CallbackQuery, state: FSMContext):
        ...
        keyboard.inline_keyboard.append(
            [
                InlineKeyboardButton(
                    text=f"{emoji} {os.path.basename(media.file_path)}",
                    callback_data=f"media_delete_{media.id}",
                )
            ]
        )
    ...
    await callback.message.edit_text(
        f"🗑️ Выбери медиа для удаления (сплав #{trip_id}):", reply_markup=keyboard
    )
    await callback.answer()
